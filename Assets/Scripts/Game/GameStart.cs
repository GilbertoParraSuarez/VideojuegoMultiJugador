...
public class GameStart : MonoBehaviour
{
    ...
    public void ScoreGoal(int team)
    {
        ...
        else
        {
            if (ballScript.lastPlayerTouch != null)
            {
                ...
                {
                    0 => $"{ballScript.lastPlayerTouch} anota!",
                    1 => $"{ballScript.lastPlayerTouch} anota!",
                };
            }
            else
            {
    ...
    private void EndGame()
    {
        if (isExtraTime)
        {
            mainText.text = (team1Score > team2Score) ? "TEAM ROSA GANA!" : "TEAM AZUL GANA!";
            winnersTeam = (team1Score > team2Score) ? 0 : 1;

            // Solo enviar puntajes finales al terminar el juego
            StartCoroutine(SendFinalScores());
    ...
    // Método para enviar puntajes finales al servidor
    private IEnumerator SendFinalScores()
    {
        // Enviar puntaje del equipo 1
        yield return StartCoroutine(SendScoreToServer("TeamRosa", team1Score));

        // Enviar puntaje del equipo 2
        yield return StartCoroutine(SendScoreToServer("TeamAzul", team2Score));
    }

    // Método para enviar puntajes al servidor PHP
    private IEnumerator SendScoreToServer(string teamName, int score)
    {
        WWWForm form = new WWWForm();
        form.AddField("team_name", teamName);
        form.AddField("score", score);

        using (UnityWebRequest www = UnityWebRequest.Post("http://localhost/bdd/Puntaje.php", form))
        {
            yield return www.SendWebRequest();

            if (www.result == UnityWebRequest.Result.ConnectionError || www.result == UnityWebRequest.Result.ProtocolError)
            {
                Debug.LogError("Error al enviar el puntaje: " + www.error);
            }
            else
            {
